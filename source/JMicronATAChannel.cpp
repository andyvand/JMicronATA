...
#define CLASS JMicronATAChannel
#define super IOService

OSDefineMetaClassAndStructors( JMicronATAChannel, IOService )
                  ...
                  IORegistryEntry * dtEntry )
{
    ...
    fProvider = OSDynamicCast(JMicronATAPCIRoot, provider);
    if (fProvider == 0)
        return false;
                                ...
                                SInt32 *       score )
{
    ...
    name1 = OSDynamicCast(OSString, table->getObject(kRootHardwareVendorNameKey));
    name2 = OSDynamicCast(OSString, fProvider->getProperty(kRootHardwareVendorNameKey));
...
{
	return fProvider->getPCIDevice();;
}

void CLASS::prepareHwForSleep( void )
{
	fProvider->prepareHwForSleep();
}

void CLASS::restoreHwResources( void )
{
	fProvider->restoreHwResources();
}
